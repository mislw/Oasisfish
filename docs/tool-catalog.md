...
| `@deepseek-ai/dsh-tool-image-generate` | `image_generate` | `ctx.tools`, `ctx.imageGeneration` | `tool/call`, `durable generated-image attachment`, `tool/result` | - | The tool uses the separately configured default image provider and model; it does not change the conversation model route. |
...
Searches one explicitly declared Skill corpus and returns relative source citations with line ranges.
