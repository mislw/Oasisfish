...
flowchart LR
  ...
  svc_imageGeneration["ctx.imageGeneration<br/>Configured image generation"]
  ...
  svc_skillSearch["ctx.skillSearch<br/>Skill knowledge search seam"]
...
| `ctx.imageGeneration` | `seam` | [`image-generation`](../packages/attachment/image-generation) | - | [`tool-image-generate`](../packages/attachment/tool-image-generate) | - | 该服务解析单独配置的 Images API 路由，并通过 `ctx.attachments` 持久化结果；工具在不改变对话模型的情况下公开此能力。 |
...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-local`](../packages/memory/memory-local) | [`tool-memory`](../packages/memory/tool-memory) | - | 服务通过单一提供方路由全局用户记录和由 cwd 派生的项目记录；工具负责模型的显式写入和已记录的首 step 上下文注入，Settings 则使用服务的 Remote 方法。 |
...
| `ctx.skillSearch` | `seam` | [`skill-search`](../packages/skill/skill-search) | [`skill-search-local`](../packages/skill/skill-search-local) | [`tool-skill-search`](../packages/skill/tool-skill-search) | - | 解析显式声明的 Skill 语料并将检索分派给作用域内的提供方；本地提供方负责建立索引与排序，tool-skill-search 则公开面向模型的查询。 |
