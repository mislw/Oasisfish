...
## Provider-route circuit breaking

`@deepseek-ai/dsh-llm-circuit-breaker` wraps `llm/stream` and keeps process-local state for each exact `GenerateOptions.provider` route. Consecutive configured transient terminal failures open only that route; open calls receive `CIRCUIT_OPEN` before provider I/O, and after the interval one synchronously reserved request probes recovery while concurrent calls remain rejected. Each physical retry attempt is observed independently, but `CIRCUIT_OPEN` is an admission decision and `dsh-llm-retry` never retries it. Timeout, retry, and circuit breaking remain separate policies. See the [package reference](../../packages/llm/llm-circuit-breaker/README.md) for configuration and lifecycle details.
