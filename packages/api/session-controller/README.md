...
`session.usageSummary({ fromInclusive, toExclusive })` reads every list-visible logical Session without activating an Agent and returns non-inherited token buckets, Turns, priced/unpriced request counts, an estimated nano-USD range, and the number of Sessions that could not be read. The explicit absolute interval lets the browser define its own local-day boundary; a failed Session is isolated instead of failing the complete summary.
