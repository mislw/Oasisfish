...
`session.usageSummary({ fromInclusive, toExclusive })` 在不激活 Agent 的情况下读取每个列表可见的逻辑 Session，并返回非继承 token 桶、轮次、已计价/未计价请求数、nano-USD 预计费用区间，以及无法读取的 Session 数量。显式绝对时间区间允许浏览器定义自己的本地日期边界；单个 Session 读取失败会被隔离，而不会让完整汇总失败。
