...
describe('desktop wallpaper engine bundle', () => {
  ...
  test('applies the audited Client compatibility patch', () => {
    ...
    expect(client).toContain('const controller = typeof AbortController === "function" ? new AbortController() : null')
    expect(client).toContain('loadPersisted(startup).then((loaded) => {')
    expect(client).toContain('startup.cancelled = true')
    expect(client).toContain('if (controller) controller.abort()')
    expect(client).not.toContain('loadPersisted().then(loadInventory);')
