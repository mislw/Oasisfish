...
The bundle also mounts the local durable-memory Provider and its Settings page. Records live under the configured JSON storage root, while prior-session full-text search uses a dedicated `session-search.sqlite` derived index opened only by the first search. The selected agent preset decides whether those Host capabilities become model-facing tools.
