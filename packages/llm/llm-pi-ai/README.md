...
## Draft connection testing

The plugin also registers `ctx.llm.registerProviderProbe('llm-pi-ai', …)` for a real one-shot generation against the endpoint, protocol, credential, model, and cancellation signal a configuration surface is currently editing. It builds a temporary single-route profile and `PiAiAdapter`; the profile never joins the live adapter registry, does not write settings or credentials, and produces no Session events. The request asks for exactly `OK`, caps output at sixteen tokens so OpenAI-compatible gateways do not reject an undersized request before model execution, disables SDK retries through the adapter, and has a 15-second overall limit. A successful test can therefore consume a small amount of provider quota.

A credential typed into the draft wins over the stored credential for an existing route; if the draft field is empty, the same stored `apiKeyEnv` resolution used by model discovery supplies the key. Credentials are validated before provider I/O and are removed if an endpoint echoes them in its assistant text. Results classify failures as endpoint, authentication, protocol, model, or response failures and return only a short sanitized message; provider response bodies and credentials never enter the result.
