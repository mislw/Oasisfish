...
- **Resolve token prices** — `ctx.llm.tokenPricing(provider, model, occurredAt)` returns the adapter's synchronous per-token billing ranges for uncached input, cache reads, optional cache writes, and output; an unknown route remains unpriced.
