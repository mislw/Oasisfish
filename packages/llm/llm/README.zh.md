...
- `ctx.llm.registerProviderProbe(settingsNs: string, probe): () => void` 为一个 settings namespace 所拥有的提供方档案注册草稿连接测试。每个 namespace 只能有一个（`INVALID_PROVIDER_PROBE`/`DUPLICATE_PROVIDER_PROBE`），并随调用 fiber dispose。
- `ctx.llm.testProvider(settingsNs: string, request: LlmProviderProbeRequest): Promise<LlmProviderProbeResult>` 测试一个仅供本次请求使用的端点、协议、凭据和模型草稿。无人服务的 namespace 以 `NO_PROVIDER_PROBE` 失败。
...
提供方连接测试同样属于配置期针对草稿的操作。运行时会把端点、协议、凭据、模型和调用方 signal 复制给已注册的 settings namespace 处理器，但不会存储草稿、修改实时路由或向 Session 日志加入任何内容。实现只返回简短且已净化的结果，并分类为 `endpoint`、`authentication`、`protocol`、`model` 或 `response`；凭据绝不能进入结果文本或诊断。
