...
Use `ctx.tokenMeter` to estimate current request and context pressure, price one message, or summarize reported usage and cost over a time interval. Measurements replay the durable Session log without model calls, so compaction, occupancy displays, and telemetry can share deterministic results. Session projections expose `tokenUsage`, `usageCost`, `contextPressure`, and `contextBreakdown`; text and routes without image pricing use a fixed heuristic, declared visual pricing applies when available, and files use model-visible handle text. Provider usage is reused only for an identical request envelope. The package adds no model-visible content and makes no loop decisions.
...
| File | Role |
|---|---|
| [`src/index.ts`](src/index.ts) | The `TokenMeter` service: replay state, fold, measurement, message estimates, and period summaries |
...
| [`src/usage-cost.ts`](src/usage-cost.ts) | Provider-priced request projection and explicit-period usage summary |
