...
Conversation 状态栏在流式生成期间显示对 CJK 感知的本轮近似 token 数，随后显示整个 Session 的轮次与步骤数、解码速度、提供方报告的 token、缓存命中率和适配器定价的预计费用区间。今日 pill 按浏览器本地自然日汇总列表可见的 Session，并展示轮次、请求数、token 桶、缓存命中、预计费用、未计价请求与无法读取的 Session；会话详情还展示最近请求的模型、token 与估算费用。回复落定后，状态栏读取持久化投影，因此分页和压缩不会改变累计数值；横向滚动确保窄屏仍可访问每个 pill。

Assistant 回复结算后，已完成轮次的计时对话框不显示 TTFT 和解码速度，实时回复完成后和重新打开历史后都如此。轮次总耗时仍可用。Session Stats 读数从其持久化投影独立获取计时。
