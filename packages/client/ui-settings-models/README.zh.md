...
模型与中转站设置及产品引导插件。同一个 client Cordis 插件会注册“模型与中转站”页面和版本化内测声明。提供方配置保留在设置页，启动时不会阻塞应用。页面把 `llm.providers`、`llm.models`、`llm.defaultModel`、`settings.describe` 与 `credentials.describe` 汇聚为一个共享快照，一次只展开一张提供方编辑卡片，并显示端点主机、协议与模型数量摘要，但不把路由存活状态呈现为提供方状态。
...
缺少提供方凭据时不会注册第二个引导步骤。会话模型目录没有可选项时，应用仍可交互，composer 的模型触发器会标明尚未配置；菜单会引导用户前往“模型与中转站”，既有提供方编辑器仍是唯一配置入口。
