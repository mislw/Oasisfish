...
**测试连接**会用同一份尚未保存的端点、协议、可选替换密钥与所选模型发送一次最小提供方请求。结果会显示完成阶段与耗时，并把端点、鉴权、协议、模型、响应失败分类，但绝不渲染所提供的密钥或提供方响应正文；界面同时提醒成功探测可能消耗少量额度。任一草稿字段变化都会清除陈旧结果。探测仅是配置操作：它不会保存 profile、改变默认值，也不会向 Session 日志追加事件。
