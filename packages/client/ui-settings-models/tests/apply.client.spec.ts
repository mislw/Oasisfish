...
describe('ui-settings-models apply', () => {
  ...
  it('the label thunk follows the active locale without re-registration', async () => {
    ...
    expect(resolveSlotLabel(b.slots.entries('settings.section')[0]!.options.label)).toBe('Models and Relays')
    ...
    expect(resolveSlotLabel(b.slots.entries('settings.section')[0]!.options.label)).toBe('模型与中转站')
    expect(injected().t('deleteTitle')).toBe('删除 {provider}？')
  })
  ...
  it('registers the zh/en nav dictionaries and disposes everything with the fiber', async () => {
    ...
    await fiber.await()
    expect(b.locale.bind('settings.models')('nav')).toBe('模型与中转站')
    await fiber.dispose()
    expect(b.slots.entries('settings.section')).toHaveLength(0)
    expect(b.slots.entries('settings.onboarding')).toHaveLength(0)
...
describe('pushed invalidations', () => {
  ...
  it('joins the refreshed mirror view on a settings invalidation', async () => {
    ...
    const b = await bench(true, { describe }, { llm: {
      providers,
      models: vi.fn(() => Promise.resolve({
        rpcId: 'apply-models-catalog' as never,
        result: { ok: true as const, value: { groups: [], failures: [] } },
      })),
      defaultModel: vi.fn(() => Promise.resolve({
        rpcId: 'apply-models-default' as never,
        result: { ok: true as const, value: { selected: { provider: 'deepseek-official', model: 'deepseek-chat' } } },
      })),
    } })
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
