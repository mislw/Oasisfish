...
Conversation 状态栏固定在滚动区下方，用于承载紧凑的 Session 读数，且不会覆盖 transcript 或常驻 composer。空间充足时读数右对齐，空间不足时首项保持可见，并可横向滚动。其中的上下文占用按钮在扩展统计项右侧显示圆环和百分比。点击按钮可在视口内的面板查看 token 构成；上下文用量和容量尚不可用时，按钮及空状态栏保持隐藏。
