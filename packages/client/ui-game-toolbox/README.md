...
The `game-ui` preset owns the eight-stage UI production process. The `game-image` preset owns the four stages `需求`, `规格`, `生成`, and `确认`; it can load `oasis-wiki`, record progress, ask one focused question, and call the configured default image model without filesystem, shell, UMG, Lua, or editor tools.
