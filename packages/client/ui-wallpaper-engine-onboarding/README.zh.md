...
浏览器插件注册类型化中英文字典，并以 order `100` 贡献一个 root-scoped 引导 row。每次 locale 变化都会用新的分离文案快照替换该 row。挂载的组件启动一个可中止请求，在 owner callback 替换时保留该请求，等待期间不渲染内容，并在请求结束后通过最新 callback 完成；警告与完成操作最多各发生一次。可见模态框拥有 `#root` 的 inert 状态、聚焦标题、拒绝隐式关闭，并在导航到设置前调用完成操作。
