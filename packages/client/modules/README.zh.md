...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希和包专属 `dsh.client.external` 请求写入启动图，把动态提供方排在消费者之前，并通过 `/plugins` 提供该文件及其 sourcemap。根 Loader 暴露 `resolvePackageJson` 时，扫描会使用它，使封闭安装运行时从插件导入所选中的同一安装自有包读取客户端元数据；普通源码运行和 profile 自有运行仍以配置树的 `baseUrl` 解析。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。启用 `@deepseek-ai/dsh-client-ui-settings-memory` 时，它对 `@deepseek-ai/dsh-api-remotes` 的显式动态边会确保生成的 `memory` Remote 在设置页面物化前挂载。
