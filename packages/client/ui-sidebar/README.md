...
Sidebar shell plugin: the brand row, New Session action, layout-owned collapse control, Sessions/Toolbox view switch, scroll-aware region seats, and bottom-pinned Settings seat. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; tool packages render into `sidebar.toolbox`. This package neither derives either view's rows nor owns their domain state. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).

The expanded brand row renders `sidebar.brand.mark` and `sidebar.brand.name` as independent single slots, while the collapsed rail renders the same mark slot. Without occupants, the shell uses the Oasisfish bitmap and an `Oasisfish` label carrying the build's 7-character `DSH_CLIENT_COMMIT_HASH` badge. A deployment package can replace either value without replacing the New Session control or rail geometry; declaration-aware `slots.inject()` lets such a package activate before or after the sidebar.
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared brand, `sidebar.workspaces`, `sidebar.toolbox`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. Component-local state selects Sessions or Toolbox; collapsing and expanding the sidebar preserves that selection. There is no plugin store.

The expanded shell keeps a low-opacity Oasisfish mascot scene below its live controls. The collapsed rail removes that decoration so the 56px control path stays quiet.
