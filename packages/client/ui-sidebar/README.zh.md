...
侧边栏外壳插件：负责品牌行、New Session 操作、布局持有的折叠控件、会话/工具箱视图切换、可感知滚动的区域 seat，以及固定在底部的 Settings seat。[ui-workspace](../ui-workspace/README.zh.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；工具包渲染到 `sidebar.toolbox`。本包既不派生两个视图中的行，也不持有其领域状态。折叠到布局拥有的 56px 轨道仍属于本地呈现行为。约定：[slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.zh.md)。

展开的品牌行把 `sidebar.brand.mark` 与 `sidebar.brand.name` 渲染为两个独立的 single slot，收起轨道则渲染同一个 mark slot。没有占位者时，外壳使用 Oasisfish 位图，以及带有构建期 7 位 `DSH_CLIENT_COMMIT_HASH` 徽标的 `Oasisfish` 标签。部署包可以单独替换任一值，而无须替换 New Session 控件或轨道几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。
...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的品牌、`sidebar.workspaces`、`sidebar.toolbox` 与 `sidebar.settings` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。组件本地状态选择会话或工具箱；收起再展开侧边栏会保留该选择。这里没有插件 store。

展开状态会在实时控件下方持续显示低透明度 Oasisfish 吉祥物场景。折叠到 56px 控制栏时会移除该装饰。
