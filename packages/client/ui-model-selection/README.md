...
Once the directory reaches `ready` with no model choices, the trigger reads `No model configured` instead of the generic selection prompt. Opening its Model pane explains that configuration belongs in Models and Relays; this is a navigation hint rather than a composer block.
