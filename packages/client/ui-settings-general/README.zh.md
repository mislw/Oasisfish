...
模态层通过 portal 挂载到 `document.body`，因此侧栏的 transform 和 backdrop filter 不会限制其全视口遮罩与居中面板。面板通过基于主题 token 的半透明阅读层和淡化的本地吉祥物标记，继续显示共享 Oasisfish 海景。
...
宿主端在用户设置 seam 中保留 `ui-onboarding.welcomeNoticeVersion`，使现有用户文档仍可通过校验。已发布的 client 不会读写该字段，外壳本身仍不持有产品策略。
