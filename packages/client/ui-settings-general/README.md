...
The modal layer is portaled to `document.body`, so sidebar transforms and backdrop filters cannot constrain its full-viewport mask or centered panel. The panel keeps the shared Oasisfish scene visible through a translucent theme-token reading surface and a faint local mascot mark.
...
The Host half retains `ui-onboarding.welcomeNoticeVersion` in the user-settings seam so existing user documents remain valid. No shipped client reads or writes this field, and the shell itself remains policy-free.
