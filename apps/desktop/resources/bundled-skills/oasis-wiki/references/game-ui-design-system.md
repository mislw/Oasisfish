...
- When the active session has no built-in `image_gen`, an official environment Key is absent, the active credential belongs to a custom provider, or the default image model is unavailable, do not stop before upstream discovery. Run `generate_with_codex_provider.py --discover-image-models` to make a metadata-only `GET /models` request. It must report `generation_attempted: false`, never perform an automatic paid probe, and list `confirmed`, `likely`, and `uncertain` candidates with evidence. Keep `selection_required: true` even for one candidate; only after developer selection and explicit authorization may `codex_provider_direct` generate. Never print or persist credentials, and never fall back to HTML/CSS/Chromium screenshots.
...
Before creating reconstruction jobs, resolve every `reuse_of`, `texture_asset`, item semantic key, and item ID against `--library-references`. Common controls such as close, back, add, tabs, currency icons, and standard action buttons must reuse their active project-library component; never replace a resolved control with generated artwork or a text glyph. Component previews must come from a real asset export, an explicitly approved transparent cutout, or a provenance-labeled editor reconstruction. Never crop a Content Browser thumbnail, selection outline, checkerboard, screenshot annotation, or generated page image into a reusable component preview. If an active `reuse_of` has no resolved library reference, stop before generation instead of redesigning it. Reuse a library bitmap only when the component is `active` and its preview file is readable. `candidate`, `pending_review`, `deprecated`, and unresolved entries remain review evidence and must not suppress reconstruction.
