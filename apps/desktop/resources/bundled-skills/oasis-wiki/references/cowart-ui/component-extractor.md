...
```powershell
python scripts/cowart-ui/component-extractor/create_ui_workbench.py --image <ui-final.png> --controls <ui-tree.json> --visual-review <visual-review.json> --library-references <resolved-library-references.json> --name "<page name>"
```

The generated preview is a composition check. Oasis Companion is the primary review surface: the script starts a read-only loopback supervisor plus worker, queues its URL and session directory in the Companion handoff inbox, and registers the generated UI in the left page navigator. If the HTTP worker exits, the supervisor restarts it on the same port. A running Companion receives the handoff without a duplicate process, retries short loopback interruptions, and falls back to the persisted page when the local server remains unavailable. Each page keeps its own image, control tree, and assets. Generating the same `page_id` again updates that navigation entry to the latest session without deleting older session directories. The printed local URL and `Open UI Workbench.url` remain browser fallbacks, not the durable session identity. The independent layer files and `layer-manifest.json` are authoritative for editing.

The page ID defaults to a stable value derived from `--name`; use `--page-id <lowercase-id>` when the caller owns a clearer durable identity. Companion persists the page catalog and selected page, so registered sessions remain available after restart without the original localhost process. If a browser tab reports `ERR_CONNECTION_REFUSED`, first confirm whether it is a stale ephemeral port; reopen the page from Companion's persisted catalog instead of treating the browser error as proof that Companion crashed. Use `--companion-executable <path>` for a nonstandard Companion installation or an isolated test binary. Use `--no-start` only when another process will manage review: it disables both the loopback server and Companion handoff.
...
8. Import only validated `clean_layer` PNGs with `insert_cowart_image`. Clean Composite parent layers may render as editable shapes, but only approved Skin/Artwork nodes may enter the reusable component library. Native nodes remain placeholders unless `reuse_of` resolves an active project-library component. A resolved native component keeps its interactive widget, records `component_reuse.status: ready` plus the Unreal `source_asset`, and displays the library preview; it is not reconstructed and must not fall back to a text glyph. Put `componentId`, `logicalParentId`, `layer`, `zIndex`, `sourceBounds`, `nodeKind`, `assetSource`, and `reviewStatus` into `shapeMeta`.
