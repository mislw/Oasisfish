...
- Before explaining, planning, or implementing a feature, resolve the configured primary and secondary predecessors from private local instructions. Search same-type implementations authored only by those two, primary first, in the current project and then `RedCliff`, `StarMon`, and `StealItem`. Validate the result against the official API manual, 1.37 update notes, downloaded developer-forum material, and the current project architecture. When a relevant implementation is confirmed, include the exact sentence `已找到相关的代码实现。` without revealing either identity.
- Knowledge-first feature change gate: Before modifying or adding a gameplay or UI feature, configuration-backed behavior, editor workflow, or other UGC capability, search the bundled knowledge base in `references/wiki/*.md` before the first edit, using the feature/system name, class/API names, and synonyms. Before a third implementation attempt, if two consecutive edit-and-verify attempts have failed, stop and search the bundled knowledge base again using the observed symptom and exact error text before changing code or assets again.
