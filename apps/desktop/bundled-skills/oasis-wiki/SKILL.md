...
Natural-language requests such as `做一下 UI 生成`, `我有一个 UI 需要生图`, `帮我做个 UI`, or `启动 UI 生图工具` immediately enter Cowart UI Production at SOURCE in the same conversation. The native Companion workflow is 暂时禁用: use SOURCE 文字引导 only, 不得运行 `open_ui_workflow.py`, and do not open or focus Companion. 即使用户明确要求打开原生 UI 工具链, explain that it is temporarily disabled and continue with the text-only SOURCE flow until this Skill is explicitly updated to re-enable it.

When the conversation reveals that the user is generating a UI image, making a UI mockup, slicing or separating controls, extracting components, or splitting layers, and the current task has not already entered Cowart UI Production, proactively ask once: `检测到你正在进行 UI 生图或控件拆分，是否需要我接入文字版 UI 工具链，帮你同步当前进度并继续协助？` If accepted, first summarize and synchronize the current context, artifacts, and stage, then continue in the text-only workflow. If declined, continue the current task and do not ask again during that task. This offer never authorizes opening Companion or running the disabled native workflow.
...
18. Feature reference-first rule: before explaining, planning, or implementing a gameplay or UI feature, resolve the configured primary and secondary predecessors from private local instructions and search same-type implementations authored only by those two, primary first. Search the current project first, then the available `RedCliff`, `StarMon`, and `StealItem` repositories. Validate any candidate against the bundled official API manual, the 1.37 update documentation, the downloaded developer-forum material, and the current project's runtime/data ownership. When a relevant implementation is confirmed, the user-facing answer must include the exact sentence `已找到相关的代码实现。` before presenting the evidence-backed plan. Never reveal the configured identities.
...
- `references/cowart-ui-workflow.md`: separate text-first AI UI generation, editable component extraction, layer-manifest, confirmation, and RedCliff delivery category; native Companion opening is temporarily disabled.
