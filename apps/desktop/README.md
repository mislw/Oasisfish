...
Desktop carries independent Python and Node.js distributions, Node's bundled npm, and the locked pnpm CLI. Python includes numpy, pandas, python-docx, python-pptx, openpyxl, Pillow, lxml and XlsxWriter with their complete dependencies. The `load_workspace_dependencies` tool installs this payload offline on first use under `$DSH_HOME/dsh-runtimes/dsh-primary-runtime` (normally `~/.dsh/dsh-runtimes/dsh-primary-runtime`) and returns absolute interpreter, pnpm script and library paths plus `pythonDistributions`, the bundled distribution names and versions. The version report excludes user-installed additions. Office tasks prefer these libraries unless user or workspace instructions select another environment. Execute the pnpm script with the returned Node executable. The returned Node library directory contains the bundled npm package and remains separate from pnpm's global installation directory.
...
Desktop also carries the complete `oasis-wiki` bundle in `runtime/bundled-skills/oasis-wiki`. The Host registers the application-owned parent directory through `DSH_BUNDLED_SKILL_DIR`, so a fresh offline installation can discover the Oasis UGC knowledge base without a separate Codex, WorkBuddy, DSH, or shared-agent skill installation. The checked-in `VERSION` marker and Desktop preparation tests keep the packaged copy explicit and reject Python bytecode or cache directories.
...
The private Desktop `runtime/bin` directory is added only to package-installation processes. The Host prepends the installed primary runtime's `dependencies/node/bin` directory to the PATH inherited by PTC and agent shells, so packaged Desktop provides `node`, `npm`, and `npx` without a system Node.js installation. The `load_workspace_dependencies` tool does not otherwise change environment variables or user package-manager configuration. pnpm retains its own defaults and user settings for global packages, executable entries and its store, including native errors when the environment does not support global installation. There is no separate dependency updater. [The primary-runtime decision](../../.agents/notes/implemented/feature/2026-09-14-desktop-primary-runtime.md) records these choices.
...
The official Desktop build uses the ordinary `web` bundle list followed by `@deepseek-ai/dsh-desktop-wallpaper-engine`. New user profiles, development projects, and packaged runtime projects receive that list. For an existing Desktop profile, `desktop-default-bundles.json` records each Desktop-only default already offered to that profile; a missing record makes startup append an absent wrapper once and then record it. Removing the wrapper later through Plugins does not cause startup to restore it.

Desktop validates an existing offer record before profile cleanup; malformed JSON, an unknown schema, duplicate names, or non-string names fail profile preparation without rewriting the manifest or record. When an offer changes activation, Desktop atomically publishes `package.json` before the state record. A state-write failure can therefore leave the manifest updated and the record missing; startup fails and releases the transaction lock, and the next launch retries without duplicating the bundle. Native recovery restores only the ordinary Web bundles and preserves the offer record, so recovery disables the wrapper and later launches do not re-enable it.
...
The Wallpaper Engine prompt appears only while the upstream `GET /wallpaper-engine/settings` response contains `settings: null`; any settings object, including an empty object, suppresses later onboarding. The upstream plugin stores its configuration under `~/.dsh-wallpaper-engine`, restores it after Host or Desktop restart, and owns wallpaper selection without a Desktop-side copy.

Disabling the wrapper in Plugins removes the upstream integration and onboarding together and remains effective after restart. Native recovery applies the same complete disablement while preserving the upstream configuration directory. Mixed-language copy inside the Wallpaper Engine settings section and platform discovery behavior remain limitations of the pinned upstream plugin.
...
The repository's default build uses the `oasisfish` client profile: the shell title and fallback sidebar brand read **Oasisfish**, the bundled Oasis UI workflow and `oasis-wiki` remain in the shared application, and the Desktop Host enables the Codex bridge preview through its application-owned overlay. `pnpm run build:official` remains the separate DeepSeek Harness artifact build.

Development Harness state defaults to `apps/desktop/.desktop-build/development/home`, the disposable npm project lives at `apps/desktop/.desktop-build/development/project`, and Electron browser data lives at `apps/desktop/.desktop-build/development/electron-user-data`. Sessions, settings, credentials, package links, and browser data therefore stay out of the user's normal Harness home. An explicit `DSH_HOME` replaces only the development Harness home. Renderer DevTools stays closed by default; Main, Renderer, and dsh Host debugging listen on ports 9229, 9222, and 9230. `DSH_DESKTOP_MAIN_INSPECT_PORT`, `DSH_DESKTOP_RENDERER_DEBUG_PORT`, and `DSH_DESKTOP_HOST_INSPECT_PORT` replace those ports, while `DSH_DESKTOP_OPEN_DEVTOOLS=1` opens the detached Renderer tools.
...
The lower-left account row displays localized availability, a spinner with download percentage, verification, readiness, or a persistent red retry action with an accessible tooltip. This Web-embedded copy follows the active in-application language; native dialogs use the Desktop shell locale. The collapsed sidebar shows a dot on its top expand button. Connection status takes priority. Selecting an available release starts downloading immediately. Successful preparation automatically opens a shell-owned restart confirmation; its packaged document loads from `dsh-app://shell`, and the product document remains blurred only while that child window is open. Closing it retains readiness without reopening the dialog. Selecting the ready entry opens confirmation again. Running agents, queued input, and running or stopping jobs trigger an interruption warning in that confirmation. API requests alone do not trigger the warning. After approval, the Host locks new requests, drains admitted requests, and rechecks tasks, including work created by an admitted write. A drain that exceeds the control-request deadline refuses installation and unlocks admission. Unknown task status, newly started work without interruption approval, or unsuccessful graceful teardown prevents installation. Ordinary quit hides the product window before stopping the Host, ignores new focus requests during teardown, and never installs an update. The next launch reconciles the version-bound runtime through the existing startup and recovery path.
