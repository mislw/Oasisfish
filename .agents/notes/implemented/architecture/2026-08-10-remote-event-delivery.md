...
The few Client-owned symbols are therefore **mirrored** on the test side when an assembled scenario needs them; the two chat e2e keep importing `dsh-client-runtime/client` because the `runtime` project is already in the Host graph. This lets those consumers leave the Host graph, and the 15 Client project references in `apps/cli/tsconfig.json` have no owner-map role.
