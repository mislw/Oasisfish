...
组装场景需要少量客户端符号时，会在测试侧**镜像**这些符号；两个 chat e2e 继续直接引入 `dsh-client-runtime/client`，因为 `runtime` 工程本来就在 Host 图中。这使相关消费方可以离开 Host 图，`apps/cli/tsconfig.json` 中的 15 条 client 工程引用也不再承担 owner-map 职责。
