...
同一个版本同时具有标准公开版本和等价的 Windows 资源版本。发布时必须一起更新三个包字段；发布工作流会验证它们的对应关系，组装完整的桌面 staging 目录，验证已构建和已 staging 的应用，然后才使用这些已准备输入发布 NSIS 安装包。标准版本仍是有效 SemVer，因此 Electron Updater 可以按大版本、日期和修订号排序。
