...
语料由部署方声明，而不是从 Skill 内容中推断。每项声明包含一个 Skill、可选提供方、相对根目录、扩展名与资源上限。Windows 桌面版只声明内置 `oasis-wiki` 的 `references` 目录。提供方把每个原文解析在已加载 Skill 的目录资源基址之下，并拒绝路径穿越、reparse point、不支持的资源类型及超出配置上限的内容。启用检索不会修改不可变的 Skill 快照。
...
单元测试覆盖受限发现、Markdown 与文本分块、CJK 词法项、本地模型校验、事务刷新、取消、混合排序、提供方 dispose（资源释放）和结构化工具错误。无密钥的组装 snapshot 加载内置 Oasis Skill fixture，并记录真实的 `skill_search` 调用与结果。桌面 staging 校验固定模型的 manifest 与哈希；打包 smoke 使用真实 ONNX 模型启动应用、执行中文 Oasis 查询、使用同一个用户数据目录重启，并确认持久索引仍可使用。
...
打包后的 agent 无需独立服务、联网下载或向中转站披露内容，即可检索 Oasis 知识库，并且每条返回片段都能在持久 transcript 中定位原文。部署方必须声明每项可检索语料，并提供不可变模型资源与可变缓存存储。首次查询需要承担本地建索引开销，安装包会增加固定模型的体积；不受支持或过大的语料会明确失败，不会退化为隐式的全目录扫描。
